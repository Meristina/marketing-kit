"""marketing — Marketing-Kit CLI. Subcommands: init, run, check, version."""
...
def _cmd_init(args) -> int:
    ...
    print(f"Initialized Marketing-Kit in {summary['target']} (harness: {summary['agent']})")
    print(f"  payload source : {summary['payload_mode']}")
    if "commands" in summary:
        print(f"  commands       : {summary['commands']} → /marketing.<name>")
    for k in ("agents", "skills", "note"):
        if k in summary:
            print(f"  {k:<14} : {summary[k]}")
    print('Next:  marketing run "<your marketing goal>"   (or use /marketing.mission in your harness)')
    return 0
...
def _cmd_run(args) -> int:
    ...
    try:
        ...
        print(f"error: {e}. `marketing run` needs the engine SDK: pip install openai-agents",
              file=sys.stderr)
        return 2
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="marketing", description="Marketing-Kit — marketing toolkit CLI.")
    p.add_argument("--version", action="version", version=f"marketing-kit {__version__}")
    sub = p.add_subparsers(dest="cmd", required=True)

    pi = sub.add_parser("init", help="scaffold .marketing/ + harness commands into a project")
    ...
    pr.add_argument("goal", help="the marketing goal, one line")
    pr.add_argument("path", nargs="?", default=".", help="project dir for missions/ output")
    pr.add_argument("--steer", action="store_true",
                    help="open the interactive (optional) Direction Check instead of auto-proceeding")
