...
Single source of truth = the payload's `.marketing/commands/*.md` (YAML frontmatter with
...
|----------|------------------------------|--------------------------|--------------------------------|
| claude   | .claude/commands/            | marketing.<n>.md         | MD + frontmatter (as-is)       |
| codex    | .codex/prompts/              | marketing-<n>.md         | MD + frontmatter (passthrough) |
| cursor   | .cursor/commands/            | marketing-<n>.md         | MD, NO frontmatter             |
| copilot  | .github/prompts/             | marketing-<n>.prompt.md  | YAML frontmatter + body        |
| gemini   | .gemini/commands/marketing/  | <n>.toml                 | TOML (description + prompt)     |
| opencode | .opencode/commands/          | marketing-<n>.md         | MD + frontmatter (description)  |
        ...
        if end != -1:
            ...
            for line in fm.splitlines():
                if line.strip().startswith("description:"):
                    ...
                    break
    description = description or f"Marketing-Kit /{name}"
...
def _install_claude(sources, target) -> dict:
    cmds = 0
    for f in _command_files(sources):
        _write(target / ".claude" / "commands" / f"marketing.{f.name}", f.read_text(encoding="utf-8"))
        cmds += 1
...
def _install_codex(sources, target) -> dict:
    ...
    n = 0
    for f in _command_files(sources):
        _write(target / ".codex" / "prompts" / f"marketing-{f.name}", f.read_text(encoding="utf-8"))
        n += 1
    return {"commands": n,
            "note": "codex: .codex/prompts/marketing-*.md -> /prompts:marketing-<name>. "
...
def _install_cursor(sources, target) -> dict:
    ...
    for name, _desc, body in _commands(sources):
        _write(target / ".cursor" / "commands" / f"marketing-{name}.md", body)  # no frontmatter
        n += 1
...
def _install_copilot(sources, target) -> dict:
    ...
    for name, desc, body in _commands(sources):
        content = f"---\ndescription: {desc}\nmode: agent\n---\n\n{body}"
        _write(target / ".github" / "prompts" / f"marketing-{name}.prompt.md", content)
        n += 1
...
def _install_gemini(sources, target) -> dict:
    ...
    for name, desc, body in _commands(sources):
        ...
        _write(target / ".gemini" / "commands" / "marketing" / f"{name}.toml", content)
        n += 1
    return {"commands": n, "note": "gemini: .gemini/commands/marketing/*.toml -> /marketing:<name> (args {{args}})"}
...
def _install_opencode(sources, target) -> dict:
    ...
    for name, desc, body in _commands(sources):
        content = f"---\ndescription: {desc}\n---\n\n{body}"
        _write(target / ".opencode" / "commands" / f"marketing-{name}.md", content)
        n += 1
    return {"commands": n, "note": "opencode: .opencode/commands/marketing-*.md ($ARGUMENTS supported)"}
